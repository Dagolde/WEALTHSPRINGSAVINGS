...
async function loadTransactions(page = 1) {
    ...
    try {
        ...
        const statsResponse = await apiCall('/v1/admin/transactions/stats');
        if (statsResponse.success) {
            updateTransactionStats(statsResponse.data);
        }
        ...
        // Load transactions
        const response = await apiCall(`/v1/admin/transactions?${params.toString()}`);
        
        if (response.success) {
            displayTransactions(response.data.data);
            updateTransactionsPagination(response.data);
        }
    } catch (error) {
        console.error('Error loading transactions:', error);
        document.getElementById('transactionsTableBody').innerHTML = `
            <tr><td colspan="9" class="error">Error loading transactions: ${error.message}</td></tr>
        `;
    }
}
...
function updateTransactionStats(stats) {
    ...
    document.getElementById('pendingTransactions').textContent = stats.pending.toLocaleString();
...
function displayTransactions(transactions) {
    ...
    tbody.innerHTML = transactions.map(transaction => `
        <tr>
            ...
            <td><strong>₦${parseFloat(transaction.amount).toLocaleString('en-NG', {minimumFractionDigits: 2})}</strong></td>
            ...
            <td>
                <div class="action-buttons">
                    ...
                    ${transaction.status === 'pending' ? `
                        <button class="btn-icon btn-success" onclick="approveTransaction(${transaction.id})" title="Approve">
                            <i class="fas fa-check"></i>
                        </button>
                        <button class="btn-icon btn-danger" onclick="rejectTransaction(${transaction.id})" title="Reject">
...
function updateTransactionsPagination(data) {
    ...
    // Previous button
    html += `<button onclick="loadTransactions(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''}>
    ...
    for (let i = 1; i <= Math.min(totalPages, 5); i++) {
        const page = i === 1 ? i : (i === 5 ? totalPages : currentPage - 2 + i);
        if (page > 0 && page <= totalPages) {
            html += `<button onclick="loadTransactions(${page})" class="${page === currentPage ? 'active' : ''}">${page}</button>`;
        }
    }
    html += '</div>';
    
    // Next button
    html += `<button onclick="loadTransactions(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''}>
...
function filterTransactions() {
    ...
    transactionFilters.type = document.getElementById('transactionTypeFilter').value;
    loadTransactions(1);
}
...
function searchTransactions() {
    ...
    searchTimeout = setTimeout(() => {
        transactionFilters.search = document.getElementById('transactionSearch').value;
        loadTransactions(1);
    }, 500);
}
...
async function viewTransaction(id) {
    try {
        const response = await apiCall(`/v1/admin/transactions/${id}`);
            ...
            showModal('Transaction Details', `
                <div class="transaction-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="value">${transaction.user?.name} (${transaction.user?.email})</span>
                    </div>
                    <div class="detail-row">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="value"><strong>₦${parseFloat(transaction.amount).toLocaleString('en-NG', {minimumFractionDigits: 2})}</strong></span>
                    </div>
                    <div class="detail-row">
                        <span class="label">Balance Before:</span>
                        <span class="value">₦${parseFloat(transaction.balance_before).toLocaleString('en-NG', {minimumFractionDigits: 2})}</span>
                    </div>
                    <div class="detail-row">
                        <span class="label">Balance After:</span>
                        <span class="value">₦${parseFloat(transaction.balance_after).toLocaleString('en-NG', {minimumFractionDigits: 2})}</span>
                    </div>
                    <div class="detail-row">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="value"><code>${transaction.reference}</code></span>
                    </div>
                    <div class="detail-row">
                    ...
                    ${transaction.metadata ? `
                        <div class="detail-row">
                            ...
                            <span class="value"><pre>${JSON.stringify(transaction.metadata, null, 2)}</pre></span>
                        </div>
                    ` : ''}
                </div>
            `);
...
async function approveTransaction(id) {
    ...
    try {
        const response = await apiCall(`/v1/admin/transactions/${id}/approve`, {
            method: 'POST'
        });
...
async function rejectTransaction(id) {
    ...
    try {
        const response = await apiCall(`/v1/admin/transactions/${id}/reject`, {
            method: 'POST',
            body: JSON.stringify({ reason })
        });
